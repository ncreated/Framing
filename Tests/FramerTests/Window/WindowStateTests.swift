...
class WindowStateTests: XCTestCase {
    ...
    func testChangingBlueprintVisibility() {
        // Given
        var state: WindowState = .mockRandom()
        state.blueprints = [
            .init(blueprint: .mockRandomWith(id: "1"), isVisible: .mockRandom()),
        ...
        let newState = reducer.reduce(currentState: state, on: .changeBlueprintVisibility(blueprintID: "2", newVisibility: newVisibility))

        // Then
        XCTAssertEqual(newState.blueprints[1].isVisible, newVisibility)
    }

    func testChangingControlsState() {
        // Given
        let state: WindowState = .mockRandom()

        // When
        let expectedState: WindowState.ControlsState = state.controls == .minimised ? .maximised : .minimised
        let newState = reducer.reduce(currentState: state, on: .changeControlsState(show: expectedState == .maximised))
